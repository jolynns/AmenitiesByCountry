...
#Set Workspace and define working files
    ...
    allCountries = arcpy.GetParameterAsText(1)
    allAmenities = arcpy.GetParameterAsText(2)
...
# Loop over all amenities making shape file for each
    for item in amenities:
        ...
        try:
            ...
            # check and see if there are items and report what is found.
            itemCount = arcpy.GetCount_management("amenitiesLayer")
            arcpy.AddMessage("The number of amenities found for %s is %s" % (item, itemCount))
        except:
